...
**Fecha:** 2026-01-17  
**Versión:** v3.1 (Handoff Completo + Addendum C2.16–C2.17)  
**Estado:** Proyecto funcional, listo para desarrollo continuo

---

## 🔥 Addendum (2026-01-17): estado real tras C2.16–C2.17

### Hitos recientes
- **C2.16 (Hardening):** taxonomía de errores, clasificador transitorio/definitivo, retry policy por fase, timeouts por fase, evidencias automáticas, `run_summary.json` + endpoint **GET /api/runs/summary**.
- **C2.17 (Decision flow):** plan congelado con `plan_id` estable, decisiones explícitas por ítem (`AUTO_UPLOAD|REVIEW_REQUIRED|NO_MATCH|SKIPPED`), ejecución estricta (solo `AUTO_UPLOAD`) sin recalcular decisiones.

### Tags de retorno
- **v2.13-readonly-stable** — READ-ONLY estable con e‑gestiona real.
- **v2.17-decision-flow-stable** — flujo de decisión estable (plan_id + decisiones + gate de ejecución).

### Próximo foco recomendado
**C2.18A — Observabilidad del matching**: cuando `AUTO_UPLOAD=0` por `NO_MATCH` y/o `local_docs_considered=0`, generar un informe determinista que explique por qué no hay matches (filtros, scope, type_id/aliases, paths, periodos, sujeto, etc.) y cómo corregirlo.
